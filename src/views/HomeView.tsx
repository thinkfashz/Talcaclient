...
export const HomeView: React.FC<HomeViewProps> = ({ setActiveTab }) => {
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 15 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -15 }} transition={{ duration: 0.5 }} className="pb-28 px-4 md:px-12 pt-24 max-w-6xl mx-auto space-y-12 md:space-y-16">
      <section className="relative flex flex-col lg:flex-row items-center gap-8 lg:gap-12">
        ...
        <div className="flex-1 space-y-5 md:space-y-7 z-10 w-full pt-2">
          ...
          <p className="font-sans text-[15px] md:text-lg text-stone-300 max-w-xl leading-relaxed">
            Una experiencia premium para clientas que quieren uñas impecables, reserva rápida y seguimiento real. Esta demo usa visuales internos originales, sin imágenes antiguas ni links externos que puedan fallar.
          </p>
          ...
          <div className="flex flex-col sm:flex-row gap-3 md:gap-4 pt-3 w-full">
            ...
            <a href={`https://wa.me/${WHATSAPP}?text=Hola%20Dharynails%2C%20quiero%20consultar%20por%20una%20hora%20disponible`} target="_blank" rel="noopener noreferrer" className="glass-card font-bold h-14 px-5 rounded-2xl flex items-center justify-center gap-2 text-[#25D366] border-[#25D366]/20 bg-[#25D366]/5 text-[15px] md:text-base cursor-pointer hover:bg-[#25D366]/10 transform hover:-translate-y-0.5 transition-all"><MessageCircle className="w-5 h-5 fill-current" /> WhatsApp Directo</a>
          </div>
        </div>
      ...
      <footer className="text-center border-t border-white/10 pt-8 pb-4">
        ...
        <p className="text-xs text-stone-600 mt-2 max-w-xl mx-auto">Visuales originales recreados en SVG para estabilidad y uso seguro. Propuesta demostrativa para reservas, fidelización y catálogo.</p>
      </footer>
    </motion.div>
  );
};
