...
const GALLERY_DATA: GalleryItem[] = [
  ...
  { id: '4', title: 'Carmesí Dulce', category: 'Luxe • Rojo', tag: 'Luxe', image: NAIL_IMAGES.gallery4 },
...
export const GalleryView: React.FC<GalleryViewProps> = ({ setActiveTab }) => {
  ...
  const filteredData = GALLERY_DATA.filter(item => activeFilter === 'Todos' || item.tag === activeFilter || item.category.includes(activeFilter));

  return (
    <motion.div initial={{ opacity: 0, scale: 0.98 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0 }} transition={{ duration: 0.4 }} className="pb-28 px-4 md:px-12 pt-24 max-w-7xl mx-auto">
      <div className="mb-6 md:mb-10 text-center mt-2 md:mt-4 space-y-3">
        ...
        <p className="font-sans text-sm md:text-base text-stone-400 max-w-xl mx-auto px-2">
          Galería estable con visuales internos de uñas. No depende de enlaces externos ni bancos de imágenes que puedan cargar mal.
        </p>
      </div>
      ...
      <div className="mb-6 md:mb-10 overflow-x-auto hide-scrollbar -mx-4 px-4 md:mx-0 md:px-0">
        <div className="flex space-x-2.5 md:space-x-3 w-max pb-3">
          {FILTERS.map(filter => (
            <button key={filter} onClick={() => setActiveFilter(filter)} className={`px-5 py-2.5 rounded-full font-sans text-[10px] md:text-xs uppercase tracking-wider font-bold transition-all duration-300 cursor-pointer border ${activeFilter === filter ? 'bg-white text-black border-white shadow-[0_4px_14px_rgba(255,255,255,0.15)]' : 'glass-card border-white/5 text-stone-400 hover:text-white hover:border-white/20'}`}>
              {filter}
            </button>
          ))}
        </div>
      ...
      <div className="columns-1 sm:columns-2 lg:columns-3 gap-6 space-y-6">
        {filteredData.map((item, i) => (
          <motion.div layout initial={{ opacity: 0, y: 15 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay: i * 0.05 }} key={item.id} className="break-inside-avoid group relative rounded-3xl overflow-hidden glass-card transition-all duration-500 hover:-translate-y-1 hover:border-white/20 mb-6 bg-black/80 shadow-lg border-white/5">
            <div className="relative w-full overflow-hidden rounded-t-3xl">
              <img src={item.image} alt={item.title} className="w-full aspect-[4/5] object-cover transition-transform duration-700 group-hover:scale-105 filter brightness-95 group-hover:brightness-100" />
              <div className="absolute inset-0 bg-gradient-to-t from-black/90 via-black/30 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex items-end p-5">
                <button onClick={() => setActiveTab('agenda')} className="w-full py-3 rounded-xl bg-white text-black font-sans font-bold text-xs md:text-sm flex items-center justify-center space-x-2 hover:bg-stone-200 transition-all cursor-pointer shadow-[0_4px_15px_rgba(255,255,255,0.15)]">
                  <span>Pedir Este Diseño</span><ArrowRight className="w-4 h-4" />
                </button>
              </div>
            </div>
